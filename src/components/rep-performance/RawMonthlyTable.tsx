...
const RawMonthlyTable: React.FC<RawMonthlyTableProps> = ({
  ...
  const prevRankings = useMemo(() => {
    ...
    const prevData = clonedData.map((item: any) => {
      ...
      // Calculate previous values with better handling for edge cases
  ...
  const getPreviousMonthName = (currentMonth: string) => {
    switch (currentMonth) {
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent className="bg-gray-800 border-white/10 text-white">
                          <p>
                            {showChangeIndicators ? (
                              <>
                                {getPreviousMonthName(selectedMonth)}: {formatCurrency(getPreviousValue(item.spend, repChanges[item.rep]?.spend || 0))}
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent className="bg-gray-800 border-white/10 text-white">
                          <p>
                            {showChangeIndicators ? (
                              <>
                                {getPreviousMonthName(selectedMonth)}: {formatCurrency(getPreviousValue(item.profit, repChanges[item.rep]?.profit || 0))}
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent className="bg-gray-800 border-white/10 text-white">
                          <p>
                            {showChangeIndicators ? (
                              <>
                                {getPreviousMonthName(selectedMonth)}: {formatPercent(item.margin - (repChanges[item.rep]?.margin || 0))}
                      ...
                      <Tooltip>
                              ...
                              accountsMatch ? (
                                ...
                                repChanges[item.rep] && Math.abs(repChanges[item.rep].activeAccounts) >= 0.1 ? (
                                  <div className="flex items-center ml-1">
                                    ...
                                    <span className="text-2xs ml-1 text-finance-gray">
                                      {formatNumber(Math.round(getPreviousValue(item.activeAccounts, repChanges[item.rep]?.activeAccounts || 0)))}
                                    </span>
                                  </div>
                                ) : (
                                  <span className="inline-flex items-center ml-1 text-finance-gray font-bold">
                        ...
                        <TooltipContent className="bg-gray-800 border-white/10 text-white">
                          <p>
                            {showChangeIndicators ? (
                              <>
                                {getPreviousMonthName(selectedMonth)}: {formatNumber(Math.round(getPreviousValue(item.activeAccounts, repChanges[item.rep]?.activeAccounts || 0)))}
                      ...
                      <Tooltip>
                          ...
                          <div className="flex items-center">
                            ...
                            {showChangeIndicators && repChanges[item.rep] && Math.abs(repChanges[item.rep].packs) >= 0.1 ? (
                              <div className="flex items-center ml-1">
                                ...
                                <span className="text-2xs ml-1 text-finance-gray">
                                  {formatNumber(Math.round(getPreviousValue(item.packs, repChanges[item.rep]?.packs || 0)))}
                                </span>
                              </div>
                            ) : showChangeIndicators ? (
                              <span className="inline-flex items-center ml-1 text-finance-gray font-bold">
                        ...
                        <TooltipContent className="bg-gray-800 border-white/10 text-white">
                          <p>
                            {showChangeIndicators ? (
                              <>
                                {getPreviousMonthName(selectedMonth)}: {formatNumber(Math.round(getPreviousValue(item.packs, repChanges[item.rep]?.packs || 0)))}
