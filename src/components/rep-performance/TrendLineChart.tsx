...
const CHART_COLORS = {
  ...
  revenue: '#60a5fa',
  packs: '#4ade80',
  margin: '#fbbf24', // Yellow color for margin
        ...
        <div className="bg-gray-800 border border-gray-700 p-3 rounded-md shadow-lg backdrop-blur-sm">
          ...
          {selectedReps.length === 0 && (
            <>
              ...
              {showRevenue && payload.find(p => p.dataKey === 'revenue') && (
            ...
            if (profitValue !== undefined || revenueValue !== undefined || packsValue !== undefined || marginValue !== undefined) {
              return (
                <div key={rep} className={`${index === 0 && selectedReps.length > 0 ? '' : 'mt-2 border-t border-gray-700 pt-2'}`}>
                  ...
                  {showRevenue && revenueValue !== undefined && (
                    <p className="text-sm" style={{ color: repColor }}>
    ...
    <Card className="bg-gray-900/40 border border-white/10 backdrop-blur-sm shadow-lg">
      <CardHeader className="pb-2">
        ...
        <CardDescription className="text-xs md:text-sm mb-3 md:mb-4 text-white/60">
          ...
          {' · '}Current month excluded to prevent scale distortion
        </CardDescription>
        <ToggleGroup 
          type="multiple" 
          value={activeToggles} 
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <YAxis 
                ...
                hide={!showProfit && !showRevenue && (showPacks || showMargin)}
              />
              {/* Packs axis */}
              {showPacks && (
                <YAxis 
              ...
              {showMargin && (
                <YAxis 
                  yAxisId={showPacks ? "margin" : "right"}
                  orientation="right"
                  tick={{ fill: 'rgba(255,255,255,0.6)' }}
                  tickFormatter={(value) => `${Math.round(value)}%`}
                  domain={getYAxisDomain.margin}
                  axisLine={showPacks ? { stroke: CHART_COLORS.margin } : undefined}
                  tickLine={showPacks ? { stroke: CHART_COLORS.margin } : undefined}
                  allowDecimals={false}
                />
              )}
              ...
              {selectedReps.length === 0 && showRevenue && (
                <Line 
                  yAxisId="left"
                  type="monotone" 
                  dataKey="revenue" 
