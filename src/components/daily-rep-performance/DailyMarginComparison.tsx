...
interface DailyMarginComparisonProps {
  data: any[];
  comparisonData?: any[];
  isLoading?: boolean;
...
const DailyMarginComparison: React.FC<DailyMarginComparisonProps> = ({ 
  data, 
  comparisonData = [],
  isLoading,
  ...
  const changes = sortedData.map(item => {
    ...
    const change = prevData ? item.margin - prevData.margin : 0;
    return { rep: item.rep, change };
  });
  
  const maxMargin = Math.max(...sortedData.map(item => item.margin));
          ...
          <div className="w-full h-full flex items-end justify-center overflow-x-auto px-1">
            <div className={`flex items-end ${isMobile ? 'space-x-1' : 'space-x-2'} pb-1`}>
              {sortedData.map(item => {
                ...
                const barColor = 'from-blue-500 to-blue-600/70';
...
export default DailyMarginComparison;