-- Migration: Monthly Performance Trends Chart Functions
-- This migration adds server-side aggregation functions specifically for the Monthly Performance Trends chart

-- Function to get monthly trends data based on time range type
...
BEGIN
    ...
    ELSIF aggregation_type = 'weekly' THEN
        ...
        SELECT 
            ...
            'Week ' || to_char(date_trunc('week', d."Date_Time"), 'WW') AS period_label,
            'Week ' || to_char(date_trunc('week', d."Date_Time"), 'WW/YYYY') AS full_period_label,
            COALESCE(SUM(d."Spend"), 0)::DECIMAL AS total_spend,
            COALESCE(SUM(d."Profit"), 0)::DECIMAL AS total_profit,
            COALESCE(AVG(CASE WHEN d."Spend" > 0 THEN (d."Profit" / d."Spend") * 100 ELSE 0 END), 0)::DECIMAL AS avg_margin,
            COUNT(DISTINCT d."Account Ref")::INTEGER AS unique_accounts,
