-- Final fix for ambiguous column - use FULL OUTER JOIN approach

DROP FUNCTION IF EXISTS get_daily_rep_performance_comparison(DATE, DATE, DATE, DATE, TEXT[], TEXT);

-- Simplified comparison function that avoids ambiguous columns
...
BEGIN
    ...
    SELECT 
        COALESCE(cp.rep_name, comp.rep_name) as rep_name,
    ...
    FULL OUTER JOIN comparison_period comp ON cp.rep_name = comp.rep_name
