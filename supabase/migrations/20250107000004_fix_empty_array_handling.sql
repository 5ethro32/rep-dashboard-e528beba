-- Improve empty array handling in department filtering
-- This ensures that empty arrays return no results consistently

-- Update get_daily_summary_metrics to handle empty arrays
...
BEGIN
    ...
    WHERE "Date_Time"::DATE BETWEEN start_date AND end_date
        AND (department_filter IS NULL OR 
             (array_length(department_filter, 1) > 0 AND "Department" = ANY(department_filter)))
        AND (method_filter IS NULL OR "Method" = method_filter);
END;
$$ LANGUAGE plpgsql;

-- Update get_daily_comparison_metrics (uses the updated summary function)
...
-- Update get_monthly_trends_data to handle empty arrays
...
BEGIN
    -- Return empty result if department_filter is an empty array
    IF department_filter IS NOT NULL AND array_length(department_filter, 1) = 0 THEN
        RETURN;
    END IF;
