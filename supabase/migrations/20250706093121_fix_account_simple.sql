-- Simplified Fix for Rep Performance Functions
-- Remove complexity that's causing comparison function to fail
...
-- Simplified rep performance function
...
BEGIN
    ...
    SELECT 
        ...
        COUNT(DISTINCT d."Account Ref")::INTEGER as active_accounts,
        COUNT(DISTINCT d."Account Ref")::INTEGER as total_accounts, -- Same as active for now
...
BEGIN
    ...
    all_reps AS (
        SELECT DISTINCT rep_name FROM (
            SELECT rep_name FROM current_period
            UNION
            SELECT rep_name FROM comparison_period
        ) combined
    )
    SELECT 
        ar.rep_name,
